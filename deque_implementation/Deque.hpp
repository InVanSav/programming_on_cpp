...
class Chunk {
 ...
 public:
  Chunk() {
    ...
    auto* init_header =
        reinterpret_cast<unsigned int*>(memory_blocks_.data());
    *init_header = CHUNK_SIZE - HEADER_SIZE;
    memory_max_block_ = init_header;
    memory_free_blocks_.insert(init_header);
  }

  bool IsInside(const unsigned int* address) const noexcept {
    const auto* start_chunk_address =
        reinterpret_cast<const unsigned int*>(memory_blocks_.data());
    const unsigned int* end_chunk_address = start_chunk_address + CHUNK_SIZE;
    return (start_chunk_address <= address) && (address <= end_chunk_address);
  }

  unsigned int* TryReserveBlock(std::size_t allocation_size) {
    ...
    const auto allocation_size_with_alignment =
        static_cast<unsigned int>(allocation_size + alignment_padding);
    ...
    unsigned int* header_address = *min_it;
    auto* new_header_address =
        reinterpret_cast<unsigned int*>(
            reinterpret_cast<unsigned int*>(header_address) +
                HEADER_SIZE +
                allocation_size_with_alignment);

    if (memory_free_blocks_.find(new_header_address)
        == memory_free_blocks_.cend()) {
      const unsigned int old_block_size = *header_address;
      const unsigned int difference = old_block_size - HEADER_SIZE;
    ...
    *header_address = static_cast<unsigned int>(allocation_size);
    ...
    return reinterpret_cast<unsigned int*>(header_address) + HEADER_SIZE;
  }

  void ReleaseBlock(unsigned int* block_ptr) {
    unsigned int* header_address = block_ptr - HEADER_SIZE;
    const unsigned int size_relized_block = *header_address;
    if ( (!memory_max_block_) || (size_relized_block > *memory_max_block_) ) {
      memory_max_block_ = reinterpret_cast<unsigned int*>(header_address);
    }
 ...
 private:
  ...
  constexpr DstIterator GetIterator(SrcIterator it) const {
    ...
    using reverse_iterator = std::set<unsigned int*>::reverse_iterator;
  ...
  Iterator TryDefragment(Iterator start_it, Iterator end_it) {
    ...
    unsigned int* current_header_address = *current_it;

    if ( (current_it != end_it) && (next_it != end_it) ) {
      unsigned int* next_header_address = *next_it;
      const unsigned int current_block_size = *current_header_address;
      const unsigned int* available_current_block_address =
          reinterpret_cast<unsigned int*>(
              reinterpret_cast<unsigned int*>(current_header_address) +
                  HEADER_SIZE +
                  current_block_size);

      if (available_current_block_address == next_header_address) {
        const unsigned int next_block_size = *next_header_address;
        const unsigned int new_current_block_size = current_block_size +
            HEADER_SIZE +
            next_block_size;
        *current_header_address = new_current_block_size;
        ...
        auto delete_it =
            GetIterator<std::set<unsigned int *>::iterator>(next_it);
 ...
 public:
  ...
  std::set<unsigned int*> memory_free_blocks_;
  unsigned int* memory_max_block_;
};
...
class CustomAllocationStrategy {
  ...
  static_assert(CHUNK_SIZE <= std::numeric_limits<unsigned int>::max(),
                "Chunk size must be less or equal max value of the unsigned int");
 ...
 public:
  void* allocate(std::size_t size) {
    ...
    unsigned int* allocated_block = chunk.TryReserveBlock(size);
    return allocated_block;
  }
  ...
  void deallocate(void* memory_ptr, std::size_t size) {
    ...
    auto* deallocation_ptr = static_cast<unsigned int*>(memory_ptr);
    for (auto& chunk : memory_chunks_) {
...
class Deque {
 public:
  ...
  reference at(size_type pos) {
    if (pos < 0 || pos > this->vector_.size()) {
  ...
  reference back() {
    return this->vector_[this->vector_.size()-1];
  }
  ...
  iterator end() noexcept {
    return iterator(this->vector_.capacity());
  }
  ...
  const_iterator end() const noexcept {
    return const_iterator(this->vector_.capacity());
  }
  ...
  void shrink_to_fit() {
    if (this->vector_.capacity() > this->vector_.size()) {
      vector_.resize(this->vector_.capacity());
    }
  }
